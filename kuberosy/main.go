...
func timed_stop(rd *ringbuf.Reader) {
	time.Sleep(60 * time.Second)
	stopper_func(rd)
}

func stopper_func(rd *ringbuf.Reader) {
	...
	f, err := os.OpenFile("syscall_associations.txt", os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0666)
	if err != nil {
		panic(err)
	}
